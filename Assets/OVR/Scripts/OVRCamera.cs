...
{		
	...
	public static Vector3 	DirectionOffset    	= Vector3.forward;
...
// * * * * * * * * * * * * *
	...
	new void Start()
	{
		...
		//CameraTextureScale = OVRRift.DistortionScale();
	...
	void OnPreRender()
	{
		...
		// Set camera texture here and clear depth buffer
	...
	void OnPostRender()
	{
		...
		// Set the render target to be the final back-buffer
	...
	void SetCameraOrientation()
	{
		...
		{			
			...
			{
				float yaw = gameObject.camera.transform.rotation.eulerAngles.y;
				Quaternion yawQ = Quaternion.AngleAxis(yaw, Vector3.up);
				gameObject.transform.parent.transform.rotation = yawQ;
			}
