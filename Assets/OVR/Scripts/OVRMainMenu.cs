...
public class OVRMainMenu : MonoBehaviour
{
	...
	public int    StepY				= 35;
	[HideInInspector]
	public int 	  StereoSpreadX 	= -40;
	
	// Spacing for variables that users can change
	[HideInInspector]
	public int 	  VRVarsSX			= 320;
	[HideInInspector]
	public int    VRVarsSY			= 380;
	[HideInInspector]
	public int    VRVarsWidthX 		= 160;
	[HideInInspector]
	public int    VRVarsWidthY 		= 20;
	...
	void GUIShowLevels()
	{
		...
		{   
			...
			for (int i = 0; i < SceneNames.Length; i++)
			{
				...
				StereoTextField (StartX, y, WidthX, WidthY, ref SceneNames[i], c);
			}
		}				
	}
	
	// ShowVRVariables
	void ShowVRVariables()
	{
		...
		// FPS
		StereoTextField (VRVarsSX, y, VRVarsWidthX, VRVarsWidthY, 
	...
	// StereoTextField
	void StereoTextField(int X, int Y, int wX, int wY, ref string text, Color color)
	{
		float ploLeft = 0, ploRight = 0;
		CameraController.GetPhysicalLensOffsets(ref ploLeft, ref ploRight); 
		int xL = X;
		int xR = (Screen.width / 2) + xL + StereoSpreadX - 
		...
		GUI.Box(new Rect(xL, y, wX, wY), text);
		GUI.Box(new Rect(xR, y, wX, wY), text);			
	}
	...
	bool ShowRiftDetected()
	{
		if(RiftPresentTimeout > 0.0f)
		{
			...
			StereoTextField (StartX, StartY, WidthX, WidthY, ref strRiftPresent, Color.white);
